...
let db= await con();
let collection = db.collection("Type_Inc")

export const getType_IncV100 = async ( req,res)=>{
    try {
...
export const postType_IncV100 = async (req,res)=>{
    try{
        if(!req.rateLimit) return;
        const id =  await increment("Type_Inc");
        ...
        await decrease("Type_Inc");
        errorcontroller(error, res);
    }
}

export const putType_IncV100 = async (req,res)=>{
...
export const deleteType_IncV100 = async (req,res)=>{
    try {
        if(!req.rateLimit) return;
        let id = req.body.id
