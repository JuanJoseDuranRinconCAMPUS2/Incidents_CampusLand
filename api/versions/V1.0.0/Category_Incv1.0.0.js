...
let db= await con();
let collection = db.collection("Category_Inc")

export const getCategory_IncV100 = async ( req,res)=>{
    try {
...
export const postCategory_IncV100 = async (req,res)=>{
    try{
        if(!req.rateLimit) return;
        const id =  await increment("Category_Inc");
        ...
        await decrease("Category_Inc");
        errorcontroller(error, res);
    }
}

export const putCategory_IncV100 = async (req,res)=>{
...
export const deleteCategory_IncV100 = async (req,res)=>{
    try {
        if(!req.rateLimit) return;
        let id = req.body.id
