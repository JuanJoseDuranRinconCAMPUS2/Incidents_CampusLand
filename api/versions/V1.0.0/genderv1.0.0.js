...
let db= await con();
let collection = db.collection("Gender")

export const getGenderV100 = async ( req,res)=>{
    try {
...
export const postGenderV100 = async (req,res)=>{
    try{
        if(!req.rateLimit) return;
        const id =  await increment("Gender");
        ...
        await decrease("Gender");
        errorcontroller(error, res);
    }
}

export const putGenderV100 = async (req,res)=>{
...
export const deleteGenderV100 = async (req,res)=>{
    try {
        if(!req.rateLimit) return;
        let id = req.body.id
