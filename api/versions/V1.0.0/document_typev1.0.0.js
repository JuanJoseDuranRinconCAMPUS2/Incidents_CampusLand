...
let db= await con();
let collection = db.collection("Document_type")

export const getDocument_typeV100 = async ( req,res)=>{
    try {
...
export const postDocument_typeV100 = async (req,res)=>{
    try{
        if(!req.rateLimit) return;
        const id =  await increment("Document_type");
        ...
        await decrease("Document_type");
        errorcontroller(error, res);
    }
}

export const putDocument_typeV100 = async (req,res)=>{
...
export const deleteDocument_typeV100 = async (req,res)=>{
    try {
        if(!req.rateLimit) return;
        let id = req.body.id
