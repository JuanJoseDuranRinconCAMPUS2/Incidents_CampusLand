...
let db= await con();
let collection = db.collection("Comp_Status")

export const getComp_StatusV100 = async ( req,res)=>{
    try {
...
export const postComp_StatusV100 = async (req,res)=>{
    try{
        if(!req.rateLimit) return;
        const id =  await increment("Comp_Status");
        ...
        await decrease("Comp_Status");
        errorcontroller(error, res);
    }
}

export const putComp_StatusV100 = async (req,res)=>{
...
export const deleteComp_StatusV100 = async (req,res)=>{
    try {
        if(!req.rateLimit) return;
        let id = req.body.id
