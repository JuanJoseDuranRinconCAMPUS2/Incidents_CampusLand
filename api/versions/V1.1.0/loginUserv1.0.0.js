...
export const LoginUserV110 = async (req, res) =>{
    ...
    try {
          ...
          res.status(201).send({status: 201, message: jwt, instructions: "In the header of the request put the header 'Authorization' and then put this key as value", User : data.id_User, rolsUser : data.Code_Rol });
  } catch (error) {
    console.log(error);
...
export const getTokenPerIdUserV110 = async ( req,res)=>{
  try {
      if(!req.rateLimit) return;
      const id = parseInt(req.query.id, 10)
      let getToken = await Tokens_Api.findOne({ Id_User_Api: id });
      res.send(getToken)
  } catch (error) {
